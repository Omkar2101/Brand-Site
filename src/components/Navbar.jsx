...
function Navbar() {
  return (
    <div className='bg-gray-300 opacity-50 fixed z-[999] w-full px-20 py-8 flex font-["Gilroy"] text-black justify-between items-center'>
        <div className='logo'>
        ...
        <div className='links flex gap-10'>
           {["service","product","abour us","contact us"].map((item,index)=>(<a className={`"text-4xl cursor-pointer capitalize " ${index ==3 &&"ml-32"}`}>{item}</a>))}

        </div>
        <div className=' '>
        <SuprSendInbox
        //  bellComponent={() => <p>MyBell</p>}
         theme={{ bell: { color: 'green' }}}
        themeType="light / dark"
       // noNotificationsComponent={() => <p>No Notifications</p>}
        
        workspaceKey={import.meta.env.VITE_SUPRSEND_WORKSPACE_KEY}
        subscriberId="tx_OrnUpvLb5DINa7lJtsPViYLuxS69IqqMLmtb5gqU"
        distinctId="rathod.6@iitj.ac.in"
        stores={[]}
        pageSize={20}
        pagination={true}
      />
